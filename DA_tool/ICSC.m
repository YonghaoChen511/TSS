function [Zs,Zt,predLabels,Z_test,test_predLabels]= ICSC(Xs,Ys,Xt,Yt,test,test_labels,options)
...
for iter = 1:T
    % solving (9)
    [Zs,Zt,Z_test] = JPDA(Xs,Xt,Ys,pseudoLabels,test,options);
    % solving (5)
    Zmean = mean([Zs;Zt]);
...
Z_test = Z_test - repmat(Zmean,[size(Z_test,1) 1 ]);
Z_test = L2Norm(Z_test);
distClassMeans = EuDist2(Z_test,classMeans);
expMatrix = exp(-distClassMeans);
probMatrix = expMatrix./repmat(sum(expMatrix,2),[1 C]);
[prob,test_predLabels] = max(probMatrix');
acc = sum(test_predLabels'==test_labels)/length(test_labels);
fprintf('test acc:%0.3f\n', acc);
end


function [Zs,Zt,Z_test] = JPDA(Xs,Xt,Ys,YtPseudo,test,options)
...
test = test*diag(sparse(1./sqrt(sum(test.^2))));
[m,n] = size(X);
ns = size(Xs,2);
nt = size(Xt,2);
C = length(unique(Ys));
...
%     K_test = kernel(ker,test,[],gamma);
%     [A_test,~] = eigs(K*((1-mu)*Rmin-mu*Rmax)*K_test'+lambda*eye(n),K_test*H*K_test',p,'SM');
%     test = A'*test;
end
...
Z_test = Z_test*diag(sparse(1./sqrt(sum(Z_test.^2))));
Z_test=Z_test';
